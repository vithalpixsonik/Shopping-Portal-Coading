...
<?php
	$em=$_SESSION['email'];
	$query = "select * from customer_register where email='$em'";
	$run_pro = mysqli_query($con, $query) or die("Error: " . mysqli_error($con));
	while($row = $run_pro->fetch_assoc()) {
		echo "<p style='text-align:center;font-size:20px'>Name : ".$row['name']."</p>";
		echo "<br><p style='text-align:center;font-size:20px'>Contact : ".$row['contact']."</p>";
		echo "<br><p style='text-align:center;font-size:20px'>Email : ".$row['email']."</p>";
	}
?>
</div>
